...
public class Partita {
	...
	*/
	...
	public Partita(int larghezza, int altezza){
		...
		//String BUCKET_NAME = "gioco-edids";
		//Region REGION = Region.EU_NORTH_1;
				
		String configFilePath = "C:\\Code\\giocofinalee\\credenziali_aws.properties"; // Percorso del file di configurazione
        Properties properties = new Properties();

        try (FileInputStream input = new FileInputStream(configFilePath)) {  //prende le info dal file chiavi.txt e le mette su properties
        ...
        //String keyName = "your-file-name.txt"; // Nome del file da salvare su S3
        //String filePath = "path/to/your/local/file.txt"; // Percorso del file locale da caricare
       ...
       // uploadFileToS3(s3, bucketName, keyName, filePath);
	...
	public void sparare() {
		if(this.isInGioco()) {
			...
			else sc.catturare(this.bersaglio);
		}
	}
            ...
            if(monete){
							...
							while (!done2){
								if (cmd2.equalsIgnoreCase("pagare")){
									...
									this.monete=false;
									zaino--;
									System.out.println("TOPO GUARDIA: 'heheh, ho un buon fiuto per gli affari...'");
									System.out.println();
	...
	private void saveGameToS3(String nomepartita) {  //salvare file su aws s3
		...
		for(int l = 0; l<sc.getLarghezza(); l++) {
    		...
    		estado = new EstadoJuego(this.bersaglio.getX(), this.bersaglio.getY(), this.bersaglio_ult.getX(), this.bersaglio_ult.getY(), this.zaino, this.pistola,
    	...
    	private void loadGameFromS3(String nomepartita) {
